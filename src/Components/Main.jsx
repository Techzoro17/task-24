...
const Main = () => {
  ...
  return (
    <div  className='  m-4 main-h  flex justify-between'>
        ...
        :<div className='grid grid-cols-2 bg-slate-400 overflow-auto w-5/12 px-14 mx-8 scrollbar'>
            {item.map((itemm)=>(
        ...
        <div className='bg-slate-400 w-5/12 mx-8'>
            <div className=' h-full' >
                ...
                <div className='w-full flex justify-center  '>{pay?<button className='bg-green-600 w-11/12 p-2 text-white rounded-xl text-xl font-semibold  ' onClick={()=>{setPay(false)}}>Back to shopping</button>:<button className='bg-green-600 w-11/12 p-2 text-white rounded-xl text-xl font-semibold  ' onClick={()=>{setPay(true)}}>Proceed to Pay</button>}</div>
            </div>
        </div>
